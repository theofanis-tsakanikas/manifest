...
**What it does not catch, stated here because the README will claim it.** A value that
violates a provider's attribute pattern. Those validators run in the *plan* phase, and a plan
needs a backend and credentials, so an offline run cannot reach them. Green here means the
configuration is well-formed and every attribute exists — **not** that every value is
acceptable. Where a name has a format AWS enforces, the offline stand-in is a `check_*.py`
script, not this.
...
added, and it goes stale silently in the direction of "validated".
"""
