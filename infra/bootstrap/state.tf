...
#
# No DynamoDB lock table. The S3 backend has locked through a lock file in the state bucket
# since Terraform 1.10 (`use_lockfile = true`), which removes a table, its capacity mode, its
# own encryption decision and one more thing to destroy — while putting the lock in the same
# place, under the same key, as the thing it protects. `required_version` in versions.tf is
# what stops an older Terraform silently applying with no lock at all.
...
# An explicit key policy, in its own resource so it can name the deploy role without the key
# and the role depending on each other.
#
# A key with no policy gets a default granting the account root full access — not wrong, and
# not written down anywhere a reviewer reads. The root statement below is not optional in the
# explicit version: omit it and the key becomes unmanageable by anyone, including the person
# who has to rotate or delete it.
...
resource "aws_s3_bucket" "state" {
  ...
  # State is the one thing here genuinely hard to reconstruct: it is the record of which real
  # resources exist. Losing it does not lose the configuration, it loses the mapping — and an
  # apply against a lost mapping creates a second estate beside the first.
  lifecycle {
    prevent_destroy = true
  }

  #checkov:skip=CKV_AWS_144:A single-Region estate. Cross-region replication would guard against losing the mapping to a Regional failure, at the cost of a second Region's storage and a replication role — and the recovery it enables has never been exercised, which makes it a control in name. Versioning stays.
